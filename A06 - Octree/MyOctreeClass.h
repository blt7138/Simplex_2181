...
class MyOctreeClass
{
	...
	MyOctreeClass* m_pParent = nullptr;// Will store the parent of current octant
	MyOctreeClass* m_pChild[8];//Will store the children of the current octant

	std::vector<uint> m_EntityList; //List of Entities under this octant (Index in Entity Manager)

	MyOctreeClass* m_pRoot = nullptr;//Root octant
	std::vector<MyOctreeClass*> m_lChild; //list of nodes that contain objects (this will be applied to root only)
...
public:
	...
	OUTPUT: class object
	*/
	MyOctreeClass(uint a_nMaxLevel = 2, uint a_nIdealEntityCount = 5);
	/*
	USAGE: Constructor
	ARGUMENTS:
	...
	OUTPUT: class object
	*/
	MyOctreeClass(vector3 a_v3Center, float a_fSize);
	/*
	USAGE: Copy Constructor
	ARGUMENTS: class object to copy
	OUTPUT: class object instance
	*/
	MyOctreeClass(MyOctreeClass const& other);
	void Init(void);
	/*
	...
	OUTPUT: ---
	*/
	MyOctreeClass& operator=(MyOctreeClass const& other);
	/*
	USAGE: Destructor
	ARGUMENTS: ---
	OUTPUT: ---
